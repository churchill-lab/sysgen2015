...
and a variety of physilogical phenotypes were measured. At sacrifice, we collected liver tissue 
...
Next, we need to load in the haplotype probabilities. These are in large Rdata files with other variables that will we used later.

```{r warning=FALSE}
load("/data/Rdata/DO192_DataforSysGenCourse.Rdata")
load("/data/Rdata/DO192_DataforSysGenCourse_update.Rdata")
```
...
```{r warning=FALSE}
rm(annotations.protein.192, covariates.protein.192, expr.rna.192, expr.protein.192)
```

First, let's look at the expression data in "expr.rna.new".  It is a large matrix with `r nrow(expr.rna.new)` rows and `r ncol(expr.rna.new)` columns. Let's look at a small part of the file.

```{r warnings=FALSE}
expr.rna.new[1:5,1:8]
```

The rows contain sample IDs (F326, F327, F329, etc.) and the columns contain Ensembl Gene IDs. So, to map one gene, we would use one coulmn of data.

Next, let's look at the distribution of one of the genes.

```{r warning=FALSE}
hist(expr.rna.new[,1], breaks = 20, main = colnames(expr.rna.new)[1])
```

Amazing! It lookes normally distributed. In fact, we have trnasformed each trait using an inverse normal (or rankZ) transformation. DOQTL contains a function called *rankZ()* that has been applied to each gene. If we leave the exrpession data untrasformed, we may have outliers that would adversely affect the QTL mapping. The rankZ transformation provides a compromise between no transformation and simply ranking the data. Value at the extremes of the distriubtion will still have more leverage than values near the mean, but not enough to disrupt the mapping. Also, when all traits have the same distribution we can run permutations on only one gene to assess significance thresholds. 

Next, let's look at the haplotype probabilities in "probs.192". "probs.192"" is a three dimensional numeric array. The `r nrow(expr.rna.new)` samples are in rows, the `r ncol(expr.rna.new)` DO founders are in columns and the `r dim(expr.rna.new)[3]` markers are in slices.
...
mtext(side = 2, line = 0.5, at = 20:1, text = rownames(expr.rna.new)[1:20], las = 1)
```

We also have a kinship matrix that was produced from the haplotype probabilities. A kinship matrix contains the degree of relatedness between each pair of mice. In this case, we have created a kinship matrix for each chromosome. We calculate the kinship matrix on each chromosome using the markers from all of the other chromosomes, but excluding the markers on teh current chromosome. For example, on Chr 1, we would calculate the kinship between mice using markers on Chr 2 through Chr X.  This has been shown to increase power for some QTL. The acronym "LOCO" stands for "Leave One Chromosome Out". Below is example code (that we do not run) showing how to calculate the kinship matrix.

```{r eval=FALSE}
K = kinship.probs(probs = probs.192, snps = snps.64K, bychr = TRUE)
```

## Mapping a Gene Exprssion Trait

We will first create a matrix of covarites. DOQTL requires a covariate called "sex" becuase it is used to map on the X chromosome. We will also use diet as a covariate. There are three important points to remember with covarites.
...
rownames(covar) = rownames(expr.rna.new)
```

Next, we need to decide which gene to map. We will map *Lrtm1* and we need to find the column number in "expr.rna.new" that contains this gene. We will look it up in the "annotations.rna.192" data.frame.
...
We can see that the Ensembl ID for Lrtm1 is `r annotations.rna.192$EnsemblID[row]`. Next we need to find that column in "expr.rna.new".

```{r warning=FALSE}
pheno.col = which(colnames(expr.rna.new) == ensid)
colnames(expr.rna.new)[pheno.col]
```
...
```{r warning=FALSE}
qtl = scanone(pheno = expr.rna.new, pheno.col = pheno.col, probs = probs.192, K = K.LOCO.192,
             addcovar = covar, snps = snps.64K)
```

This produces a doqtl object. Next, we plot the genome scan.
...
The peaks here is wider and the pattern of allele effects is less clear.
...
Although we genotyped the DO mice at ~77,000 markers, we now have 40,000,000 SNPs in the DO mice. We can use these to perform association mapping at each SNP. While it is possible to perform genome-wide association mapping, this take more time than we have in this tutorial. Here will will perform association mapping in the region of the local eQTL for *Lrtm1* on Chr 14. To do this, we will *assoc.map()*, which takes a similar set of arguments to *scanone()*.

```{r warning=FALSE}
assoc = assoc.map(pheno = expr.rna.new, pheno.col = pheno.col, probs = probs.192, K = K.LOCO.192[[14]],
...
If you got a row contaiing annotation data, then your gene is in the DO liver expression data.
...
Expression data often contain thousands of transcripts. In these examples using 192 samples, each call to *scanone()* takes about 1 minute.  There are `r ncol(expr.rna.new)` genes in this data set. `r ncol(expr.rna.new)` x 1 minute / 60 minutes / hour = 418.5 hours = 17.4 days.  This would be time consuming on a single computer. However, many institutions have computing clusters and cloud based computing services allow you to run many jobs at the same time. 

In order to make the task manageable, we transform each expression trait using a rankZ (or inverse normal) transformation. We then estimate significance thresholds using permutations orof a single rankZ transformed phenotype. Since all expression traits have the same distribution, we can use the same threshold for all genes.

Below, we include code that maps all traits and write the DOQTL objects out to binary Rdata files.

```{r eval=FALSE}
perms = scanone.perm(pheno = expr.rna.new, pheno.col = 1, probs = probs.192,
...
annotations.rna.new = annotations.rna.new[colnames(expr.rna.new),]

# Set the output directory.
setwd("/hpcdata/dgatti/ShortCourse/eQTL")

for(i in 1:ncol(expr.rna.new)) {

  start.time = proc.time()[3]
  qtl = scanone(pheno = expr.rna.new, pheno.col = i,
        probs = probs.192, K = K.LOCO.192, addcovar = covar, 
        snps = snps.64K)
  saveRDS(qtl, file = paste0(colnames(expr.rna.new)[i], "_",
...
stopifnot(rownames(annotations.rna.new) == colnames(expr.rna.new))
