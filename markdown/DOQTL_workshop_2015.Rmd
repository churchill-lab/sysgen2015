...
First, let's look at the expression data in "expr.rna.192".  It is a large matrix with `r nrow(expr.rna.192)` rows and `r ncol(expr.rna.192)` columns. Let's look at a small part of the file.

```{r warnings=FALSE}
expr.rna.192[1:5,1:8]
```
...
Next, let's look at the haplotype probabilities in "probs.192". "probs.192"" is a three dimensional numeric array. The `r nrow(expr.rna.192)` samples are in rows, the `r ncol(expr.rna.192)` DO founders are in columns and the `r dim(expr.rna.192)[3]` markers are in slices.
...
mtext(side = 2, line = 0.5, at = 20:1, text = rownames(expr.rna.192)[1:20], las = 1)
```
...
Expression data often contain thousands of transcripts. In these examples using 192 samples, each call to *scanone()* takes about 1 minute.  There are `r ncol(expr.rna.192)` genes in this data set. `r ncol(expr.rna.192)` x 1 minute / 60 minutes / hour = 418.5 hours = 17.4 days.  This would be time consuming on a single computer. However, many institutions have computing clusters and cloud based computing services allow you to run many jobs at the same time. 
